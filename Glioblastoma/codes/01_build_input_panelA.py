...
Keeps glioblastoma *cell lines* (OncoTree GB) that have RNA-seq.
Drops genes with log2(TPM+1) < 1 in every line.
"""
...
from src.preprocess import drop_low_genes, expression_to_genes_by_samples, pick_gbm_models

RAW = GBM / "data"
PROCESSED = GBM / "data" / "processed"
PROCESSED.mkdir(parents=True, exist_ok=True)
OUT_MATRIX = PROCESSED / "input_panelA_glioblastoma_ccle_logtpm_filtered.csv"
...
def main():
    expr_path = find_file(RAW, ["OmicsExpressionProteinCodingGenesTPMLogp1.csv"])
    model_path = find_file(RAW, ["Model.csv"])
    if expr_path is None or model_path is None:
        print("Missing Model.csv or OmicsExpression*.csv in", RAW)
        return 1
    ...
    mask, rule = pick_gbm_models(models)
    if "ModelType" in models.columns:
        mask = mask & models["ModelType"].astype(str).str.contains(
            "cell line", case=False, na=False
        )
        rule = rule + "; ModelType=Cell Line"

    gbm_models = models.loc[mask].copy()
    gbm_ids = set(gbm_models[id_col].astype(str))
    print(f"Metadata filter ({rule}): {len(gbm_ids)} models")
    print(f"Expression: {expr_path.name}")
    print(f"Models:     {model_path.name}")

    expr = pd.read_csv(expr_path, index_col=0)
    expr = expression_to_genes_by_samples(expr)
    keep_ids = [c for c in expr.columns if str(c) in gbm_ids]
    missing = sorted(gbm_ids - set(map(str, expr.columns)))
    ...
    used = gbm_models[gbm_models[id_col].astype(str).isin(keep_ids)].copy()
    ...
    OUT_REPORT.write_text(
        "\n".join(
            [
                ...
                f"id_column: {id_col}",
                f"filter: {rule}",
                f"n_models_in_metadata: {len(gbm_ids)}",
