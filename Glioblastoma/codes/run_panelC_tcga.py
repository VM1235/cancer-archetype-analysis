#!/usr/bin/env python3
"""GBM Panel C, Groves protocol, TCGA Verhaak labels.
  ...
  5. Scatter + variance-explained vs tumor-only PCA and gene shuffles.

Tumor colors are GeneExp_Subtype (Verhaak), not Panel B cell-line calls.
"""
...
SUGGESTED = GBM / "results" / "panel_a" / "suggested_k.txt"
...
ARC_COLORS = ["#4C78A8", "#F58518", "#E45756", "#72B7B2", "#54A24B"]
...
def main():
    ...
    k = int(SUGGESTED.read_text().strip().splitlines()[0])
    arcs_path = GBM / "results" / "panel_a" / f"archetypes_k{k}_parti.npy"
    ...
    if len(labeled):
        ...
        print("\nNearest archetype, rows=Verhaak:")
        print(ct.to_string())
    ...
    fig.suptitle("Figure 1C  —  TCGA-GBM (Verhaak), Groves Panel C protocol", y=1.02)
    fig.tight_layout()
    fig.savefig(FIG / "Figure_1C_gbm.png", dpi=200)
    plt.close(fig)
    print("Wrote", FIG / "Figure_1C_gbm.png")
