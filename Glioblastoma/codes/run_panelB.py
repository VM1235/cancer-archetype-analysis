#!/usr/bin/env python3
"""GBM Panel B: Verhaak subtype enrichment vs Panel A vertices.

The polytope is the full GBM cell-line fit. Enrichment drops Neural if that
class is too small (Wang 2017 also dropped Neural as a separate RNA type).
Labels come from 02_assign_verhaak.py, not from PCHA.
"""
...
def plot_enrichment(table, path, n_bins):
    ...
    fig.suptitle(
        "Figure 1B  —  Verhaak-style marker enrichment on GBM cell lines",
        y=1.06,
        fontsize=13,
    )
    fig.tight_layout()
...
def main():
    ...
    suggested = (GBM / "results" / "panel_a" / "suggested_k.txt").read_text().splitlines()
    k_star = int(suggested[0])
    print(f"Suggested k from Panel A: {k_star} ({suggested[1] if len(suggested) > 1 else ''})")
    arcs_path = GBM / "results" / "panel_a" / f"archetypes_k{k_star}_parti.npy"
    ...
    plot_enrichment(table, FIG / "Figure_1B_gbm.png", n_bins)
    ...
    print("Wrote", FIG / "Figure_1B_gbm.png")
    return 0
