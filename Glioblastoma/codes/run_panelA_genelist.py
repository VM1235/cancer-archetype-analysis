#!/usr/bin/env python3
"""GBM Panel A on Wang 2017 signature genes (parallel to official A).

Writes results/panel_a_genelist/ only. Does not touch results/panel_a/.
Same ParTI params as official GBM A: numIter=50, 500 shuffles, delta=0.
"""
...
OUT = GBM / "results" / "panel_a_genelist"
FIG = GBM / "figures"
...
def draw_figure(expr, pca, scores, n_pcs, esv, tab, k_star, n_samples):
    ...
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
    ax.set_title(f"Archetype space ({n_samples} lines, Wang 2017 genes)")

    fig.suptitle(
        "Figure 1A  —  GBM Wang2017-restricted (ParTI PCHA, 500 shuffles)",
        y=0.995,
        fontsize=11,
    )
    FIG.mkdir(parents=True, exist_ok=True)
    fig.savefig(FIG / "Figure_1A_gbm_genelist.png")
    plt.close(fig)
    print("Wrote", FIG / "Figure_1A_gbm_genelist.png")
...
def main():
    ...
    print(f"GBM Wang2017-restricted matrix: {n_genes} genes × {n_samples} cell lines")
    print(
