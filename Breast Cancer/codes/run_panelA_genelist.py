#!/usr/bin/env python3
"""Breast Panel A on PAM50-gene restricted matrix (parallel to official A).

Writes results/panel_a_genelist/ only. Does not touch results/panel_a/.
...
MATRIX = BREAST / "data" / "processed" / "input_panelA_pam50_genelist.csv"
OUT = BREAST / "results" / "panel_a_genelist"
FIG = BREAST / "figures"
...
def main():
    ...
    print(f"Breast PAM50-restricted matrix: {n_genes} genes × {n_samples} cell lines")
    print(
    ...
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
    ax.set_title(f"Archetype space ({n_samples} lines, PAM50 genes)")

    fig.suptitle(
        "Figure 1A  —  Breast PAM50-restricted (ParTI PCHA, 500 shuffles)",
        y=0.995,
        fontsize=11,
    )
    fig.savefig(FIG / "Figure_1A_breast_genelist.png")
    plt.close(fig)
    print("Wrote", FIG / "Figure_1A_breast_genelist.png")
