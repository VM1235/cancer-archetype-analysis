#!/usr/bin/env python3
"""Breast Panel C, Groves protocol, TCGA histopathology labels.

Groves (repo: Cell-line-tumor-batch-correction-and-clustering.Rmd +
Thomas-Tumors-Bulk-Archetypes.ipynb; our SCLC scripts/run_panel_c.py):

  1. Merge cell lines + tumors on shared genes.
  2. ComBat, batch = cell-line vs tumor, mod = ~1, ref.batch = cell_line
     (their saved matrix is bc2, not fsva).
  3. Fit a NEW PCA on the ComBat-combined matrix. Do not refit PCHA.
  4. Map Panel A cell-line archetypes from gene space into that PCA.
  5. Scatter (PC1–PC2) + variance-explained vs tumor-only PCA and gene shuffles.

Labels here are IHC ER/HER2 from the TCGA clinical matrix, not PAM50 / Panel B.

Usage:
  .venv/bin/python "Breast Cancer/codes/run_panelC_tcga.py"
  .venv/bin/python "Breast Cancer/codes/run_panelC_tcga.py" --n-shuffle 20
"""
...
SCORES_PATH = BREAST / "results" / "panel_a" / "pc_scores.npy"
ARCS_PATH = BREAST / "results" / "panel_a" / "archetypes_k4_parti.npy"
N_PCS_PATH = BREAST / "results" / "panel_a" / "n_pcs.txt"
...
def main():
    ...
    fig.suptitle("Figure 1C  —  TCGA-BRCA (IHC ER/HER2), Groves Panel C protocol", y=1.02)
    fig.tight_layout()
    fig.savefig(FIG / "Figure_1C_tcga.png", dpi=200)
    plt.close(fig)
    print("Wrote", FIG / "Figure_1C_tcga.png")
