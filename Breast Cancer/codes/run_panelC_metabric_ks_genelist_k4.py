#!/usr/bin/env python3
"""METABRIC Panel C on KS genelist archetypes, FORCED to k=4.

Reads prepared METABRIC tables from panel_c_metabric_ks_genelist/ (inputs only).
Uses extended-k Panel A (6 PCs) from panel_a_ks_genelist_extendedk/.
Writes panel_c_metabric_ks_genelist_k4/; does not touch panel_c_metabric_ks_genelist/.
"""
...
SCORES_PATH = PANEL_A / "pc_scores.npy"
N_PCS_PATH = PANEL_A / "n_pcs.txt"
...
COMBAT_SRC = BREAST / "results" / "panel_c_metabric_ks_genelist" / "CCLE_METABRIC_COMBAT.csv"
OUT = BREAST / "results" / "panel_c_metabric_ks_genelist_k4"
FIG = BREAST / "figures"
...
def run_combat(merged, batch, force, prefer_r=True):
    ...
    if COMBAT_SRC.exists() and not force:
        print("Reusing ComBat from k=3 METABRIC run:", COMBAT_SRC)
        shutil.copy2(COMBAT_SRC, out_path)
        return pd.read_csv(out_path, index_col=0)
...
def main():
    ...
    if not arcs_path.exists():
        raise FileNotFoundError(
            f"Missing {arcs_path}. Run: python run_panelA_ks_genelist_extendedk.py --only-k 4"
        )
    ...
    n_vol = arcs.shape[1]
    k = arcs.shape[0]
    print(f"Forcing k={k_star} (extended-k Panel A; see extendedk t-ratio table)")
    ...
    ax.set_title("METABRIC tumors in cell-line archetype space (k=4)")
    ...
    fig.suptitle("Figure 1C  —  METABRIC-BRCA, KS-restricted archetypes (k=4)", y=1.02)
    fig.tight_layout()
    fig.savefig(FIG / "Figure_1C_metabric_ks_genelist_k4.png", dpi=200)
    plt.close(fig)
    print("Wrote", FIG / "Figure_1C_metabric_ks_genelist_k4.png")
