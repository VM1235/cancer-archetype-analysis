#!/usr/bin/env python3
"""Run_2 Panel B: PAM50 enrichment on Run_2 k=4 vertices, LumA/Normal dropped.

The polytope is still the 63-line Panel A fit. Only the enrichment sample
set excludes LumA (n=4) and Normal (n=1). PAM50 calls are reused from Run_1;
...
SCORES = BREAST / "results" / "panel_a" / "pc_scores.npy"
...
def plot_enrichment(table, path, n_bins, n_arcs):
    ...
    fig.suptitle(
        "Figure 1B  —  PAM50 enrichment (LumA and Normal removed)",
        y=1.06,
        fontsize=13,
    )
    fig.tight_layout()
...
def main():
    ...
    suggested = (BREAST / "results" / "panel_a" / "suggested_k.txt").read_text().splitlines()
    k_star = int(suggested[0])
    print(f"Suggested k from Panel A: {k_star} ({suggested[1] if len(suggested) > 1 else ''})")
    arcs_path = BREAST / "results" / "panel_a" / f"archetypes_k{k_star}_parti.npy"
    if k_star != 4:
        print(
            f"Note: Panel B will use k={k_star} vertices because that is the "
            "updated Panel A choice, not a hardcoded k=4."
        )
    ...
    plot_enrichment(table, FIG / "Figure_1B_breast.png", N_BINS, n_arcs)
