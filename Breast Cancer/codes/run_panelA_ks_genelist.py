#!/usr/bin/env python3
"""Breast Panel A on KS (Tan et al 2014) generic cell-line signature restricted matrix (parallel to official A).

Writes results/panel_a_ks_genelist/ only. Does not touch results/panel_a/.
...
OUT = BREAST / "results" / "panel_a_ks_genelist"
FIG = BREAST / "figures"
...
def main():
    ...
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
    ax.set_title(f"Archetype space ({n_samples} lines, KS genes)")
    ...
    fig.savefig(FIG / "Figure_1A_breast_ks_genelist.png")
    plt.close(fig)
    print("Wrote", FIG / "Figure_1A_breast_ks_genelist.png")
