#!/usr/bin/env python3
"""Breast Panel A with Groves/ParTI PCHA parameters, matrix-specific dim.

See docs/PARAMS_vs_Groves.txt.

PCHA branch of ParTI.m (algNum=5):
  numIter=5
  observed: 3*numIter = 15 inits, max volume
  each shuffle: numIter = 5 inits, max t-ratio
  maxRuns: 500 here (Groves used 1000)
  delta=0, fit in (k-1) PCs

dim: smallest n_pcs with >=50% gene-space variance on THIS breast matrix
     (Groves used 12 PCs on SCLC because that was ~47–50%).
"""
...
def main():
    ...
    print(f"Breast matrix: {n_genes} genes × {n_samples} cell lines")
    print(
    ...
    fig.suptitle(
        "Figure 1A  —  Invasive breast CCLE (ParTI PCHA, 500 shuffles)",
        y=0.995,
        fontsize=11,
    )
